...
CS5330 Project~5: \texttt{conv1}~($1{\to}10$, $5{\times}5$, stride~1),
max-pool~$2{\times}2$, ReLU; \texttt{conv2}~($10{\to}20$,
...
portable to the original T4 target.
...
Given more time, the natural next step is closing the gap to cuDNN
on larger convolution shapes where its dispatch overhead is
amortized. At LeNet scale the comparison favors the hand-written
direct kernels --- cuDNN's per-launch fixed cost ($\sim\!70\,\mu s$
on this platform) exceeds the entire convolution it is dispatching
--- but on the $32{\to}64$, $64\times64$ synthetic shape cuDNN
already pulls ahead, and on production-scale convolutions (hundreds
of channels, large spatial extents) its performance rests on
Winograd-style algorithmic reformulations for $3\times3$ and
$5\times5$ filters~\cite{lavingray2016}, Tensor-Core mixed-precision
paths on Ampere and later, and heavily hand-tuned kernels per shape.
A direct-convolution kernel like ours has no access to any of that.
A productive continuation would replace the direct convolution with
an \texttt{im2col} plus GEMM formulation, reusing our tiled GEMM,
and benchmark the resulting pipeline against cuDNN on the same
shapes. A second promising direction is operator
