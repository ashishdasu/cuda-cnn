...
The proposal named one peer-reviewed paper; the final submission
includes five (LeCun et~al., Krizhevsky et~al., Lavin and Gray,
Volkov and Demmel, Chetlur et~al.) to satisfy the rubric's three-paper
minimum and cover each major technical thread in the work.
...
Three observations from this work are worth stating directly.
...
The beyond-proposal extensions produced two concrete findings beyond
the throughput numbers. First, the fused \texttt{conv+bias+ReLU+pool}
kernel demonstrated that launch-overhead elimination is a real and
measurable lever at LeNet's smallest shapes ($1.76\times$ at
\texttt{conv1} batch~1), even when shared-memory tiling is not.
Second, the cuDNN numeric case study (Section~\ref{sec:cudnn})
surfaced a non-obvious hardware fact: Ada Lovelace's
\texttt{CUDNN\_DEFAULT\_MATH} silently selects TF32 tensor cores,
whose 10-bit mantissa produces $\sim\!5\times10^{-3}$ errors that
pass unnoticed in coarse benchmarks but exceed this project's
$1\times10^{-4}$ parity requirement by more than an order of magnitude.
Matching numerics required pinning the math type to
\texttt{CUDNN\_FMA\_MATH} and the algorithm to
\texttt{IMPLICIT\_PRECOMP\_GEMM} --- neither documented in the standard
cuDNN quickstart --- and is a concrete example of why ``just use the
vendor library'' is not a numerical free-win on modern GPUs.
Taken together, all three project tiers were completed:
minimum (naive forward pass, $1.91\times10^{-5}$ over 10,000 images,
100\% argmax agreement), target (tiled kernels, $3.6\times$ at
batch-64 \texttt{fc1}), and beyond-proposal (fused kernel and cuDNN
baseline). The tiled pipeline inferences at $147{,}100$ images/second
at batch~64 on an RTX~4090.
