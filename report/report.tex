...
\begin{abstract}
We build a forward-pass inference engine for a LeNet-style MNIST
...
a result we attribute to the small working set fitting in L1 and
...
classifier from the author's prior project work (two conv layers, two
max pool layers, two fully connected layers, ReLU activations, and a
log-softmax output). Correctness is specified as matching a PyTorch
forward pass on the MNIST test inputs to within a documented
absolute-error tolerance of $1\times10^{-4}$ per log-softmax element.
...
On the GPU-kernel side, two lines of work inform the target-tier
optimizations. Volkov and Demmel~\cite{volkov2008} characterized GPU
...
interpreting the gap between our direct kernels and a cuDNN baseline,
which selects Winograd or implicit-GEMM algorithms per shape. Chetlur
\emph{et~al.}~\cite{cudnn2014} describe cuDNN itself, which is used
in the stretch tier as a throughput reference.
...
\centering
\begin{tikzpicture}[
  tnode/.style={draw, semithick, rounded corners=1pt, fill=black!4,
                minimum height=0.85cm, minimum width=0.95cm,
                align=center, inner sep=2pt, font=\small},
  shp/.style={font=\scriptsize\ttfamily, text=black!75},
  opl/.style={font=\scriptsize\itshape, align=center, text=black!80,
              inner sep=1pt},
  ar/.style={-{Stealth[length=2.2mm,width=1.7mm]}, semithick},
]
  \node[tnode] (x) {$\mathbf{x}$};
  \node[shp, below=2pt of x] {1{\texttimes}28{\texttimes}28};

  \node[tnode, right=1.85cm of x] (h1) {$\mathbf{h}_1$};
  \node[shp, below=2pt of h1] {10{\texttimes}12{\texttimes}12};

  \node[tnode, right=1.85cm of h1] (h2) {$\mathbf{h}_2$};
  \node[shp, below=2pt of h2] {20{\texttimes}4{\texttimes}4};

  \node[tnode, right=1.4cm of h2] (v) {$\mathbf{v}$};
  \node[shp, below=2pt of v] {320};

  \node[tnode, right=1.55cm of v] (u) {$\mathbf{u}$};
  \node[shp, below=2pt of u] {50};

  \node[tnode, right=1.7cm of u] (y) {$\hat{\mathbf{y}}$};
  \node[shp, below=2pt of y] {10};

  \draw[ar] (x)  -- node[opl, above] {\texttt{conv1} $\to$ \texttt{pool} $\to$ \texttt{ReLU}} (h1);
  \draw[ar] (h1) -- node[opl, above] {\texttt{conv2} $\to$ \texttt{pool} $\to$ \texttt{ReLU}} (h2);
  \draw[ar] (h2) -- node[opl, above] {flatten}                                                (v);
  \draw[ar] (v)  -- node[opl, above] {\texttt{fc1} $\to$ \texttt{ReLU}}                        (u);
  \draw[ar] (u)  -- node[opl, above] {\texttt{fc2} $\to$ \texttt{log\_softmax}}                (y);

  \node[draw, dashed, thin, rounded corners=2pt,
        inner sep=6pt, fit=(x) (h1) (h2), label={[font=\scriptsize\itshape, text=black!60]below:convolutional stages}] {};
  \node[draw, dashed, thin, rounded corners=2pt,
        inner sep=6pt, fit=(v) (u) (y), label={[font=\scriptsize\itshape, text=black!60]below:fully-connected stages}] {};
\end{tikzpicture}
...
The target network is the LeNet-style model from the author's
CS5330 Project~5, summarized in
...
Correctness is enforced by a three-layer oracle chain that isolates
failures to the smallest possible unit before integration. The first
layer is a CPU reference implementation (\texttt{host/reference.cpp}),
written in plain C++17 with no tuning and no parallelism, covering
every operator in the network. This reference is itself validated
against five PyTorch log-softmax fixtures captured from the trained
model on MNIST test indices $0$--$4$; the maximum absolute
error is on the order of $4\times10^{-6}$, which is the
noise floor introduced by different float32 summation orders between
PyTorch and our reference loop. The CPU reference is the single
oracle against which every subsequent CUDA kernel is judged.
...
float32 range. We therefore subtract the per-row maximum before
exponentiation:
...
convolution and the linear layer. Both variants live in separate
translation units from the naive kernels and are validated against
the same CPU reference at the same $1\times10^{-4}$ tolerance before
being promoted into the end-to-end pipeline.
...
\begin{table}[htbp]
\caption{Per-kernel linear / GEMM throughput on RTX~4090. Per-launch
mean over 200 reps with warmup.}
...
synth ($N{=}1024$, $1024{\to}1024$) & 3.4153 & 0.5310 & $6.43\times$ \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
--- larger than our project tolerance, and not comparable to the
...
tiled kernel is $6.43\times$ faster than the naive one-thread-per-output
version. At batch-1 inference sizes the output tile is $1\times10$
...
example by fusing \texttt{conv+bias+pool+ReLU} into a single kernel
--- is the biggest remaining end-to-end lever and is discussed in
...
\raisebox{0pt}[0pt][0pt]{\raisebox{0pt}{\texttildelow}}99KB
shared memory/SM, 16 blocks/SM).

\begin{table}[tb]
\caption{Per-kernel static resource usage and theoretical occupancy on
RTX~4090 (compute capability 8.9). Dynamic shared-memory columns are
reported for the LeNet shapes ($5{\times}5$ filter for conv2d\_tiled,
$C{=}10$ for log\_softmax).}
...
\toprule
Kernel & Block & Regs & Static & Dyn. & Occ.\\
       &       &      & smem   & smem & \\
\midrule
conv2d\_naive      & $16{\times}16$ & 39 & 0      & 0      & 100\% \\
conv2d\_tiled      & $16{\times}16$ & 40 & 0      & 1600\,B & 100\% \\
linear\_naive      & $256$          & 38 & 0      & 0      & 100\% \\
linear\_tiled      & $16{\times}16$ & 37 & 2048\,B & 0      & 100\% \\
max\_pool2d\_naive & $16{\times}16$ & 20 & 0      & 0      & 100\% \\
relu\_naive        & $256$          & 8  & 0      & 0      & 100\% \\
log\_softmax\_naive & $16$          & 15 & 0      & 64\,B  & 17\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
story, exactly as the per-kernel discussion there describes. The
outlier is \texttt{log\_softmax\_naive} at 17\% occupancy, because it
launches one block per row with \texttt{next\_pow2}$(C)=16$ threads
for a 10-class output; the 16-block/SM ceiling is hit first. At
LeNet's batch-1 output of a single 1\,$\times$\,10 row the kernel
runs at $\sim$1\,\textmu s and is not worth specializing further.

\subsection{Case Study: Matching Numerics Against cuDNN}
...
truncates the mantissa to 10~bits, and on our conv2 shape this
...
Getting cuDNN to produce numerics comparable to our direct-convolution
...
The proposal named one peer-reviewed paper; the rubric
requires at least three, so two additional references are added
(Lavin and Gray, CVPR~2016; Volkov and Demmel, SC~2008). The
proposal described ``softmax'' as the final nonlinearity, but the
...
portable to the original T4 target. Performance measurement uses
the proposal's Nsight Compute plan is only partially realized, for
container-permission reasons detailed at the top of
Section~\ref{sec:kernel-resource}: the static half (registers,
shared memory, launch bounds, theoretical occupancy) is reproduced
via \texttt{nvcc -Xptxas=-v} and hand-computed occupancy in
Table~\ref{tab:kernel-resource}, and the timing half is covered by
the 200-rep \texttt{cudaEvent} harness in \texttt{bench.cu}, which
at LeNet scale resolves the $\sim\!1\,\mu s$ differences between
variants seen in the tables above. The two hardware-counter readouts
that Nsight Compute would have added --- SM pipe utilization and
DRAM throughput per kernel --- are not reported. The stretch-tier goal named in the proposal (``meaningful
speedup toward cuDNN on convolution throughput'') was not achieved
in the direction originally intended --- at LeNet scale our direct
kernel beats cuDNN by $7{-}25\times$ because of cuDNN's fixed
per-launch overhead, not because of any algorithmic advantage on our
side. The cuDNN comparison is therefore reported as a calibration
...
$1024\times1024\times1024$ workload the naive one-thread-per-output
kernel takes $3.42$~ms and the tiled variant takes $0.53$~ms, a
$6.4\times$ speedup. The same tiled GEMM at LeNet inference shapes
...
entire LeNet-scale convolution it is dispatching, so at our target
...
our kernel, and at production-scale convolutions the gap would
continue to widen for reasons that are structural rather than
tuning-effort questions. First, algorithm: cuDNN selects
Winograd-family reformulations for $3\times3$ and $5\times5$ filters
that cut the arithmetic count by a factor of two or more compared to
the direct formulation~\cite{lavingray2016}; our kernels implement
the textbook convolution and cannot recover that arithmetic.
Second, hardware path: on Ada Lovelace the FP16/TF32 tensor cores
deliver roughly an order of magnitude more throughput than the FP32
CUDA cores our kernels are restricted to, and cuDNN can dispatch to
them under permissive math modes (we intentionally disabled that to
preserve numerical parity; see Section~IV.D). Third, shape
specialization: cuDNN ships a library of hand-tuned kernels selected
by heuristic per $(C_{\text{in}}, C_{\text{out}}, H, W, K)$ tuple,
while this project has a single generic tile geometry intended to
run on every shape. Fourth, memory layout: vectorized NHWC paths can
exploit 128-bit loads that NCHW cannot, and cuDNN is free to
transpose into whichever layout wins for a given shape; we stay in
NCHW throughout to match PyTorch byte-for-byte. Each of these is a
meaningful project on its own; the comparison in
Table~\ref{tab:throughput-conv} is therefore presented as a
reference point that establishes the crossover shape, not as a
stretch-tier goal that was reached.

The natural next step at the scope of this project is not any of the
above but \texttt{im2col} plus GEMM --- a data rearrangement that
turns convolution into a matmul and reuses our existing tiled GEMM,
giving a third \texttt{conv2d} variant to compare without writing a
new compute kernel. A second promising direction is operator
fusion: the current pipeline has ten kernel launches per image, and
\texttt{conv+bias+ReLU} and \texttt{conv+bias+pool+ReLU} are natural
fusion candidates that would shrink launch overhead and the
host-to-device memory traffic that currently dominates the
batch-1 end-to-end time.
...
% Optional 4th, for the cuDNN baseline paragraph. Often cited from
% arXiv but the IEEE-preferred form is the ACM/IEEE workshop if used.
\bibitem{cudnn2014}
