...
as tiled shared-memory variants. A three-layer correctness harness
validates each kernel against a CPU reference and the composed pipeline
against PyTorch on five MNIST fixtures. On an NVIDIA RTX~4090, the
full GPU forward pass agrees with PyTorch to a maximum absolute error
of $1.91\times10^{-5}$ over the entire $10\,000$-image MNIST test set,
with $97.93\%$ accuracy and 100\% argmax agreement.
Per-kernel benchmarks show tiled GEMM reaching $6.43\times$ over naive
at $1024\times1024\times1024$; tiled convolution slightly regresses at
LeNet scale because the small working set fits in L1, making tiling's
synchronization cost dominant. I also implemented a cuDNN baseline to
see how the direct kernels compare to the vendor library, which turns
out to be slower at LeNet shapes due to descriptor and dispatch
overhead, and only pulls ahead on a larger $32{\to}64$, $64\times64$
synthetic workload.
\end{abstract}
...
log-softmax output). Convolution is the right primitive here because
images have spatial structure: nearby pixels are correlated, useful
features like edges and textures appear at multiple locations, and a
convolutional filter can detect them anywhere in the image without
learning a separate weight for each position. The correctness target is matching a PyTorch forward pass on the
...
work covers naive CUDA kernels for all five operators, tiled
shared-memory variants for convolution and the linear layer, and a
three-layer correctness harness that validates each kernel against a
CPU reference before it enters the full pipeline. Beyond the core scope, I also implemented a fused conv+bias+ReLU+pool
kernel that eliminates two intermediate DRAM round-trips per conv stage,
a cuDNN baseline that required explicit algorithm and precision-mode
configuration to produce numerically comparable results
(Section~\ref{sec:cudnn}), and a compiler-level occupancy analysis
characterizing register, shared-memory, and warp utilization for all
eight kernels (Section~\ref{sec:kernel-resource}). Each produced a
finding that was not obvious before the work was done.
...
its small parameter count (approximately $21\,800$ weights in the
...
ReLU, \texttt{fc2}~($50{\to}10$), log-softmax. The two convolutional
stages learn a hierarchy of spatial features --- \texttt{conv1}
learning low-level detectors such as edges and stroke orientations,
\texttt{conv2} combining those into higher-level digit-part
representations --- before the fully connected layers map that
representation to class scores. The eight learnable
...
kernels remain portable to T4, A100, and 30-/40-series GPUs.
...
predicted argmax agrees with PyTorch on every image. 
...
identical to PyTorch's eval-mode accuracy on the same weights.
Human error on MNIST is approximately $0.2\%$; $97.93\%$ is
consistent with published results for LeNet-5 variants at this
parameter scale~\cite{lecun1998}.
...
Nsight Compute was the intended profiling tool. In the
...
throughput numbers in Section~\ref{sec:throughput}: the naive-vs-tiled delta at
...
\label{sec:cudnn}
Implementing the cuDNN baseline surfaced a numerical issue that is
easy to miss and worth calling out. Ada Lovelace GPUs (compute capability~8.9,
...
reference before it was integrated, the end-to-end
...
kernel is correct and the tile geometry is reasonable --- but that these shapes are just too small to exercise what tiling is actually good at. The exception that proves the rule is the
...
Beyond the throughput numbers, these additional investigations each
produced a finding that was not obvious going in. The fused
...
The engine matches PyTorch to $1.91\times10^{-5}$ over 10,000 images
with 100\% argmax agreement and runs at $147{,}100$~images/second at
batch~64 on an RTX~4090 --- fast enough to classify every frame of
roughly 4,900 simultaneous 30~fps video streams in real time. The tiled kernels delivered $3.6\times$ over
naive at batch-64 \texttt{fc1}. Going beyond the core scope, the fused
kernel delivered a real $1.76\times$ win at \texttt{conv1} batch~1
where it mattered, and the cuDNN work uncovered the TF32 precision
trap described above --- the kind of thing that only shows up when you
actually try to match numerics against the vendor library.
